...
const attemptClockwiseRotation = ({ board, player, setPlayer }) => {
  const shape = rotate({
...
const attemptAntiClockwiseRotation = ({ board, player, setPlayer }) => {
  const shape = rotate({
    piece: player.tetromino.shape,
    direction: 1,
  });
...
export const playerController = ({
  ...
  if (action === Action.RotateClockwise) {
    attemptClockwiseRotation({ board, player, setPlayer });
  } else if (action === Action.RotateAntiClockwise) {
    attemptAntiClockwiseRotation({ board, player, setPlayer });
  } else {
